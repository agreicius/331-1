...
},
{
  ...
  "body": " Smith normal form: Euclidean domains   Let be a Euclidean domain with Euclidean size function , and let . To reduce to the diagonal matrix described in , proceed as follows.   Step 1  If , stop. Otherwise use row and\/or column operations to replace the first entry of with , any entry of minimal size with respect to the size function .    Step 2  Use row and column operations together with the division algorithm to replace all other entries besides in the first row and first column with zeros. If at some point the division algorithm produces a nonzero element of size less than , then return to Step 1.    Step 3  Let be the submatrix of consisting of all rows besides the first, and all columns besides the first. If for some , bring into the first column using a column swap and return to Step 1 applied to . Otherwise, we have for all and return to Step 1 applied to .    Using , let for , and let for an isomorphism . We have      "
},
{
